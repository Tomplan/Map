import{r as n,s as _}from"./index-12ba77bc.js";function S(c=new Date().getFullYear()){const[p,u]=n.useState({saturday:[],sunday:[]}),[m,y]=n.useState(!0),[h,f]=n.useState(null),v=n.useRef(c);n.useEffect(()=>{v.current=c},[c]);const s=n.useCallback(async t=>{try{y(!0),f(null);const e=t!==void 0?t:v.current;let r=_.from("event_activities").select(`
        ...
        `).eq("event_year",t).order("display_order",{ascending:!0});if(r)throw r;const a=(e==null?void 0:e.filter(i=>i.day==="saturday"))||[],o=(e==null?void 0:e.filter(i=>i.day==="sunday"))||[];return{data:{saturday:a,sunday:o},error:null}}catch(e){return console.error("Error loading archived activities:",e),{data:null,error:e.message}}},[]),k=n.useCallback(async t=>{try{const{data:e,error:r}=await _.from("event_activities").select("*").eq("event_year",t);if(r)throw r;if(!e||e.length===0)return{data:null,error:"No activities found for source year"};const a=e.map(d=>{const{id:l,created_at:x,updated_at:L,...q}=d;return{...q,event_year:c}}),{data:o,error:i}=await _.from("event_activities").insert(a).select();if(i)throw i;return await s(),{data:o,error:null}}catch(e){return console.error("Error copying activities from previous year:",e),{data:null,error:e.message}}},[c,s]);n.useEffect(()=>{s()},[s]),n.useEffect(()=>{s()},[c,s]),n.useEffect(()=>{let t=null;return(!(typeof navigator<"u")||navigator.onLine)&&(t=_.channel(`event-activities-changes-${c}`).on("postgres_changes",{event:"*",schema:"public",table:"event_activities",filter:`event_year=eq.${c}`},e=>{e.eventType==="INSERT"&&e.new?u(r=>{var i;const a=e.new.day;return((i=r[a])==null?void 0:i.some(d=>d.id===e.new.id))||s(),r}):s()}).subscribe()),()=>{t&&_.removeChannel(t)}},[c,s]),n.useEffect(()=>{const t=()=>s();return window.addEventListener("eventActivitiesUpdated",t),()=>window.removeEventListener("eventActivitiesUpdated",t)},[s]);function C(t,e){if(t.location_type==="exhibitor"&&t.companies){const r=t.companies;return{text:r.name,boothNumber:null,companyId:r.id}}return{text:e==="nl"?t.location_nl:e==="de"?t.location_de:t.location_en,boothNumber:null,companyId:null}}return{activities:p,loading:m,error:h,getActivityLocation:C,createActivity:g,updateActivity:b,deleteActivity:w,archiveCurrentYear:E,loadArchivedActivities:A,copyFromPreviousYear:k,refetch:s}}export{S as u};
