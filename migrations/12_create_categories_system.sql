...
-- Insert initial categories with translations (idempotent - skips if exists)
DO $$
DECLARE
  cat_vehicles UUID;
  cat_camping UUID;
...
BEGIN
  ...
  ON CONFLICT (slug) DO UPDATE SET icon = EXCLUDED.icon, color = EXCLUDED.color, sort_order = EXCLUDED.sort_order
  ...
  ON CONFLICT (category_id, language) DO UPDATE SET name = EXCLUDED.name, description = EXCLUDED.description;
