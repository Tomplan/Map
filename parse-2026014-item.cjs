...
async function run() {
  ...
  const result = await parsePdfInvoice(file, ["kraam"]); 
  
  console.log("EXTRACTED RAW LINE ITEMS:", JSON.stringify(result.line_items, null, 2));
}
run().catch(console.error);
`;
fs.writeFileSync('./parse-item-runner.cjs', code);
