...
function parseSpatialInvoice(items, allowedItems) {
  ...
  lines.forEach((l,i)=>{
    const txt = l.map(it=>it.str).join(' ');
    if (/opmerking/i.test(txt)) console.log(i, txt);
  });
...
async function run(filename = './public/invoices/2025108.pdf') {
  ...
  console.log("== EXTRACTED PARSED OBJECT ==");
