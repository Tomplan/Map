...
function makeChain(table) {
  ...
  const chain = {
    ...
    then(fn) {
      const payload = getQueryResponse(table, 'select') ?? getQueryResponse(table, 'eq') ?? getQueryResponse(table, 'in') ?? []
      return Promise.resolve({ data: payload, error: null }).then(fn)
    },
    catch() { return Promise.resolve({ data: getQueryResponse(table, 'select') ?? getQueryResponse(table, 'eq') ?? [], error: null }) }
  }
