    ...
    <>
      ...
      >
        ...
        {role && (
          <div className="px-6 py-2 bg-gray-50 border-b border-gray-200">
            <span className="text-sm text-gray-600">
              {t('helpPanel.yourRole')}{' '}
              <span className="font-semibold text-blue-600">
                {t(`helpPanel.roles.${role}`, role)}
              </span>
            </span>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          ...
          {activeTab === 'current' && (
            <div className="space-y-6">
              <div>
                <h3 className="text-2xl font-bold text-gray-800 mb-2">{currentPageHelp.title}</h3>
              ...
              <div className="prose prose-sm max-w-none text-gray-700 text-left">
                <ReactMarkdown
                  components={{
                    ...
                    h1: ({ node, ...props }) => (
                      <h1
                        className="text-xl font-bold text-gray-900 mt-4 mb-2 text-left"
                        {...props}
                      />
                    ),
                    h2: ({ node, ...props }) => (
                      <h2
                        className="text-lg font-bold text-gray-900 mt-4 mb-2 text-left"
                        {...props}
                      />
                    ),
                    h3: ({ node, ...props }) => (
                      <h3
                        className="text-base font-bold text-gray-900 mt-3 mb-2 text-left"
                        {...props}
                      />
                    ),
                    h4: ({ node, ...props }) => (
                      <h4
                        className="text-sm font-semibold text-gray-900 mt-3 mb-2 text-left"
                        {...props}
                      />
                    ),
                    // Style paragraphs
                    p: ({ node, ...props }) => (
                      <p className="text-gray-700 leading-relaxed mb-4 text-left" {...props} />
                    ),
                    // Style lists
                    ul: ({ node, ...props }) => (
                      <ul
                        className="list-disc list-outside mb-4 space-y-2 text-left ml-5"
                        {...props}
                      />
                    ),
                    ol: ({ node, ...props }) => (
                      <ol
                        className="list-decimal list-outside mb-4 space-y-2 text-left ml-5"
                        {...props}
                      />
                    ),
                    li: ({ node, ...props }) => (
                      <li className="text-gray-700 text-left pl-2" {...props} />
                    ),
                    // Style code
                    code: ({ node, ...props }) => (
                      <code
                    ...
                    <ul className="space-y-2">
                      {item.changes.map((change, changeIdx) => (
                        <li key={changeIdx} className="flex items-start gap-2">
                          <span
                            className={`text-xs px-2 py-0.5 rounded-full font-medium ${getChangeTypeBadge(change.type)}`}
                          >
          ...
          {activeTab === 'interactive-tour' && (
            <div className="space-y-4">
              <div>
                <h3 className="text-2xl font-bold text-gray-800 mb-2">
                  {t('helpPanel.interactiveTourTitle')}
                </h3>
              ...
              <div className="space-y-4">
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <h4 className="font-semibold text-blue-900 mb-2">
                    {t('helpPanel.steps.step1Title')}
                  </h4>
                  <p className="text-sm text-blue-800">
                    {t('helpPanel.steps.step1Text', {
                      role: t(`helpPanel.roles.${role}`, t('helpPanel.roles.unknown')),
                    })}
                  </p>
                </div>
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  <h4 className="font-semibold text-green-900 mb-2">
                    {t('helpPanel.steps.step2Title')}
                  </h4>
                  <p className="text-sm text-green-800">{t('helpPanel.steps.step2Text')}</p>
                </div>

                <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                  <h4 className="font-semibold text-purple-900 mb-2">
                    {t('helpPanel.steps.step3Title')}
                  </h4>
                  <p className="text-sm text-purple-800">{t('helpPanel.steps.step3Text')}</p>
                </div>

                <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                  <h4 className="font-semibold text-orange-900 mb-2">
                    {t('helpPanel.steps.step4Title')}
                  </h4>
                ...
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                  <h4 className="font-semibold text-gray-900 mb-2">
                    {t('helpPanel.steps.step5Title')}
                  </h4>
                  <p className="text-sm text-gray-700">{t('helpPanel.steps.step5Text')}</p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="border-t border-gray-200 px-6 py-3 bg-gray-50">
          <p className="text-xs text-gray-500 text-center">{t('helpPanel.footer')}</p>
        </div>
      </div>
    </>
  );
