...
jest.mock('../HelpPanel', () => ({
  default: ({ isOpen, initialTab }) => (
    <div data-testid="help-panel-mock" data-open={isOpen} data-initial={initialTab}>
      {isOpen ? 'OPEN' : 'CLOSED'}
    </div>
  ),
}));
...
jest.mock('../../contexts/OnboardingContext', () => ({
  useOnboarding: () => ({
    lastCompletedTour: { id: 'tour-1', source: 'help' },
    clearLastCompletedTour: jest.fn(),
  }),
}));
...
describe('AdminLayout reopen behavior', () => {
  ...
  test('effect does NOT open help when lastCompletedTour.source !== help (isolated negative case)', async () => {
    ...
    jest.mock('../../contexts/OnboardingContext', () => ({
      useOnboarding: () => ({
        lastCompletedTour: { id: 'tour-1', source: 'ui' },
        clearLastCompletedTour: jest.fn(),
      }),
    }));
