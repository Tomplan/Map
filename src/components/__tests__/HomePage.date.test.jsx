...
jest.mock('react-i18next', () => ({ useTranslation: () => ({ t: (k) => k, i18n: { language: 'en-US' } }) }));
...
jest.mock('../../hooks/useCountViews', () => ({ useSubscriptionCount: () => ({ count: 5, loading: false }) }));
...
jest.mock('../../utils/getLogoPath', () => ({ getLogoPath: (p) => p, getResponsiveLogoSources: (p) => ({ src: p, srcSet: null, sizes: null }) }));
jest.mock('../../utils/getDefaultLogo', () => ({ getDefaultLogoPath: () => '/assets/logos/4x4Vakantiebeurs.png' }));
...
describe('HomePage date rendering', () => {
  it('uses DB-driven dates when present', () => {
    mockUseEventMap.mockReturnValue({ settings: { event_start_date: '2026-10-10', event_end_date: '2026-10-11' }, loading: false });
