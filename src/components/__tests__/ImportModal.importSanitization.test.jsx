...
describe('ImportModal import sanitization', () => {
  ...
  test('does not send _categorySlugs or _translations to DB insert/update', async () => {
    ...
    const row = { ID: 1, 'Company Name': 'SanitizeCo', 'Categories': 'cat1, cat2', 'Contact Person': 'Joe' }
    ...
    const mockSupabase = {
      ...
      from: jest.fn((table) => {
        if (table === 'categories') {
          return { select: jest.fn(() => Promise.resolve({ data: [{ id: 1, slug: 'cat1' }, { id: 2, slug: 'cat2' }], error: null })) }
        }
        if (table === 'companies') {
          return {
            insert: jest.fn((payload) => {
              lastCompanyInsert = payload
              return { select: jest.fn(() => Promise.resolve({ data: [{ id: 900 }], error: null })) }
            })
          }
        }
    ...
    jest.doMock('../../contexts/DialogContext', () => ({ __esModule: true, useDialog: () => ({ toastError: jest.fn(), toastSuccess: jest.fn(), toastWarning: jest.fn() }), DialogProvider: ({ children }) => children }))

    const { default: ImportModal } = require('../common/ImportModal')

    const { container, getByText } = render(<ImportModal isOpen={true} onClose={() => {}} dataType={'companies'} existingData={[]} />)
