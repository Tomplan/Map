...
jest.mock('../../hooks/useTranslatedCompanyInfo', () => ({ useTranslatedCompanyInfo: () => 'Some translated info' }));
jest.mock('../../hooks/useCategories', () => ({ useCategories: () => ({ getCompanyCategories: async () => [] }) }));
jest.mock('react-i18next', () => ({ useTranslation: () => ({ i18n: { language: 'en' } }) }));
jest.mock('../../contexts/FavoritesContext', () => ({ useFavoritesContext: () => ({ isFavorite: () => false, toggleFavorite: () => {} }) }));
