    ...
    <section className="bg-white rounded-lg shadow p-4 mb-6" aria-labelledby="year-scope-title">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          <h3 id="year-scope-title" className="text-lg font-semibold text-gray-900">{tSafe('admin.yearScope.title', 'Year-scoped data')}</h3>
          <div><YearScopeBadge scope="year" /></div>
        </div>

        <div className="flex items-center gap-2">
          <label htmlFor="year-scope-select" className="text-sm text-gray-600 mr-2">{tSafe('admin.yearScope.viewingYear', 'Viewing year')}</label>
          <select
            value={localYear}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <a href="/admin/subscriptions" className="p-3 border rounded-lg hover:bg-blue-50 transition-colors" aria-label={`${t('admin.yearScope.subscriptions')} ${counts.subscriptions ?? '-'}`}>
          <div className="text-sm text-gray-500">{tSafe('admin.yearScope.subscriptions', 'Subscriptions')}</div>
        ...
        <a href="/admin/assignments" className="p-3 border rounded-lg hover:bg-blue-50 transition-colors" aria-label={`${t('admin.yearScope.assignments')} ${counts.assignments ?? '-'}`}>
          <div className="text-sm text-gray-500">{tSafe('admin.yearScope.assignments', 'Assignments')}</div>
      ...
      {onPreview && (
        <div className="mt-4 text-sm text-gray-600 flex items-center justify-end">
          <button id="year-scope-preview" onClick={onPreview} className="px-3 py-1.5 bg-white border rounded hover:bg-gray-50" aria-controls="year-change-preview">{tSafe('admin.yearScope.preview', 'Preview')}</button>
        </div>
      )}
    </section>
  );
