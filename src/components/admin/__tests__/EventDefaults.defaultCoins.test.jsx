...
// Simple i18n mock
jest.mock('react-i18next', () => ({ useTranslation: () => ({ t: (k, fallback) => fallback || k }) }));
...
jest.mock('../../../hooks/useOrganizationSettings', () => () => ({
  settings: {
    ...
    default_lunch_sun: 8,
    notification_settings: { emailNotifications: true, newSubscriptionNotify: true, assignmentChangeNotify: true }
  },
  loading: false,
  error: null,
...
jest.mock('../../../contexts/DialogContext', () => ({ useDialog: () => ({ toastError: jest.fn(), toastWarning: jest.fn(), confirm: async () => true }) }));
