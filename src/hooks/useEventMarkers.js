  ...
  const loadMarkers = useCallback(async (online) => {
    ...
    entry.loadPromise = (async () => {
      try {
        ...
        // build the mergedMarkers exactly as before...
        // (omitted here for brevity, original logic remains unchanged)
  ...
  useEffect(() => {
    ...
    if (!entry.windowHandlers) {
      ...
      const handleOffline = () => {
        ...
        entry.listeners.forEach((l) => l(entry.state));
        const cachedData = typeof window !== 'undefined' ? localStorage.getItem('eventMarkers') : null;
        if (cachedData) {
    ...
    makeChannel(`markers-core-changes-${eventYear}`,
      { event: '*', schema: 'public', table: 'markers_core', filter: `event_year=eq.${eventYear}` },
      () => { entry.reloadTimeout && clearTimeout(entry.reloadTimeout); entry.reloadTimeout = setTimeout(() => loadMarkers(true), 500); }
    );
    makeChannel(`markers-appearance-changes-${eventYear}`,
      { event: '*', schema: 'public', table: 'markers_appearance', filter: `event_year=eq.${eventYear}` },
      () => { entry.reloadTimeout && clearTimeout(entry.reloadTimeout); entry.reloadTimeout = setTimeout(() => loadMarkers(true), 500); }
    );
    makeChannel('markers-appearance-defaults-changes',
      { event: '*', schema: 'public', table: 'markers_appearance', filter: 'event_year=eq.0' },
      () => { entry.reloadTimeout && clearTimeout(entry.reloadTimeout); entry.reloadTimeout = setTimeout(() => loadMarkers(true), 500); }
    );
    makeChannel(`markers-content-changes-${eventYear}`,
      { event: '*', schema: 'public', table: 'markers_content', filter: `event_year=eq.${eventYear}` },
      () => { entry.reloadTimeout && clearTimeout(entry.reloadTimeout); entry.reloadTimeout = setTimeout(() => loadMarkers(true), 500); }
    );
    makeChannel('markers-assignments-changes',
      { event: '*', schema: 'public', table: 'assignments' },
      (payload) => {
        if (payload.eventType === 'DELETE') {
          entry.reloadTimeout && clearTimeout(entry.reloadTimeout);
          entry.reloadTimeout = setTimeout(() => loadMarkers(true), 500);
    ...
    makeChannel('companies-changes',
      { event: '*', schema: 'public', table: 'companies' },
      (payload) => {
        if (payload.eventType === 'UPDATE' && payload.new) {
          entry.state.markers = entry.state.markers.map((m) =>
            m.companyId === payload.new.id
              ? { ...m, name: payload.new.name, logo: payload.new.logo, website: payload.new.website, info: payload.new.info }
              : m,
          );
          if (typeof window !== 'undefined') {
    ...
    makeChannel(`event-subscriptions-changes-${eventYear}`,
      { event: '*', schema: 'public', table: 'event_subscriptions', filter: `event_year=eq.${eventYear}` },
      () => { entry.reloadTimeout && clearTimeout(entry.reloadTimeout); entry.reloadTimeout = setTimeout(() => loadMarkers(true), 500); }
    );
    ...
    return () => {
      ...
      if (entry.refCount <= 0) {
        ...
        useEventMarkers.cache.delete(eventYear);
      }
      if (entry.reloadTimeout) clearTimeout(entry.reloadTimeout);
    };
  }, [/* no deps: all captured */]);
