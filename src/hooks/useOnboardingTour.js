  ...
  const forceCleanup = useCallback((instance) => {
    try {
      ...
      if (cleanupErrors.length > 0) {
        console.warn(
          `Tour cleanup completed with ${cleanupErrors.length} minor errors:`,
          cleanupErrors,
        );
      } else {
  ...
  const initializeDriver = useCallback(
    (stepsOverride) => {
      ...
      if (driverInstance.current) {
        ...
        try {
          forceCleanup(driverInstance.current);
        } catch (e) {
          /* ignore */
        }
      }
              ...
              const missingElements = [
                {
                  ...
                  fallback: '[data-testid="year-selector"]',
                },
                {
                  selector: '.stats-grid',
                  content: 'Statistics Grid',
                  fallback: '[data-testid="stats-grid"]',
                },
                {
                  selector: '.event-totals',
                  content: 'Event Totals',
                  fallback: '[data-testid="event-totals"]',
                },
                {
                  selector: '.quick-actions',
                  content: 'Quick Actions',
                  fallback: '[data-testid="quick-actions"]',
                },
                {
                  selector: '.admin-sidebar',
                  content: 'Admin Sidebar',
                  fallback: '[data-testid="admin-sidebar"]',
                },
                {
                  selector: '.help-button',
                  content: 'Help Button',
                  fallback: '[data-testid="help-button"]',
                },
                {
                  selector: '.tab-navigation',
                  content: 'Tab Navigation',
                  fallback: '[data-testid="tab-navigation"]',
                },
                {
                  selector: '.language-selector',
                  content: 'Language Selector',
                  fallback: '[data-testid="language-selector"]',
                },
                {
                  selector: '.leaflet-container',
                  content: 'Map Container',
                  fallback: '[data-testid="map-container"]',
                },
                {
                  selector: '.favorites-toggle',
                  content: 'Favorites Toggle',
                  fallback: '[data-testid="favorites-toggle"]',
                },
              ];
              ...
              if (popover?.wrapper) {
                ...
                if (!wrapper._tourDelegationAttached) {
                  const handleWrapperClicks = (ev) => {
                    try {
                      const nextBtn =
                        ev.target.closest?.('.driver-popover-next-btn') ||
                        (ev.target.matches && ev.target.matches('.driver-popover-next-btn')
                          ? ev.target
                          : null);
                      const prevBtn =
                        ev.target.closest?.('.driver-popover-prev-btn') ||
                        (ev.target.matches && ev.target.matches('.driver-popover-prev-btn')
                          ? ev.target
                          : null);
                      ...
                      if (
                        ...
                        typeof driverInstance.current.moveNext === 'function'
                      ) {
                      ...
                      if (
                        prevBtn &&
                        driverInstance.current &&
                        typeof driverInstance.current.movePrevious === 'function'
                      ) {
                  ...
                  try {
                    wrapper._tourDelegationAttached = true;
                  } catch (e) {
                    /* ignore */
                  }
                  try {
                    wrapper.setAttribute('data-tour-handler', '1');
                  } catch (e) {
                    /* ignore */
                  }
                }
  ...
  const start = useCallback(
    async (opts = {}) => {
      try {
        ...
        if (
          !tourConfig.steps ||
          !Array.isArray(tourConfig.steps) ||
        ...
        const requiredSteps = tourConfig.steps.filter(
          (step) => step.element && step.element !== 'body',
        );
        ...
        const missingElements = requiredSteps.filter((step) => {
          ...
          const fallbackExists = fallbackSelector
        ...
        try {
          console.debug(
            '[onboarding:start] tourId=',
            tourConfig?.id,
            'requiredSteps=',
            requiredSteps.map((s) => s.element),
            'present=',
            requiredSteps.map((s) => !!document.querySelector(s.element)),
          );
        } catch (e) {
        ...
        if (requiredSteps.length > 0 && missingElements.length === requiredSteps.length) {
          ...
          const waitForTargets = (ms = waitMs, checkInterval = 100) =>
            new Promise((resolve) => {
              let elapsed = 0;
              let timeoutId = null;
              let intervalId = null;
              ...
              const check = () => {
                try {
                  const found = requiredSteps.some(
          ...
          try {
            ...
            try {
              console.debug(
                '[onboarding:start] waitForTargets result=',
                appeared,
                'missingElementsAtTimeout=',
                requiredSteps
            ...
            if (!appeared) {
              console.warn(
                'Tour elements not found (all required targets missing):',
                requiredSteps.map((s) => s.element),
              );
              if (options.onMissingTargets) {
                try {
                  options.onMissingTargets(requiredSteps.map((s) => s.element));
                } catch (e) {
                  /* ignore */
                }
              }
        ...
        const presentSteps = fullDriverSteps.filter(
          (s) => !s.element || s.element === 'body' || elementExists(s.element),
        );
        const missingSteps = fullDriverSteps.filter(
          (s) => s.element && s.element !== 'body' && !elementExists(s.element),
        );
        ...
        if (missingSteps.length > 0) {
          ...
          if (forceAbortOnMissing) {
            console.warn(
              'Tour start aborted because some required steps are missing:',
              missingSteps.map((s) => s.element),
            );
            if (opts.onMissingTargets) {
              try {
                opts.onMissingTargets(missingSteps.map((s) => s.element));
              } catch (e) {
                /* ignore */
              }
            }
          ...
          if (presentSteps.length === 0) {
            ...
            if (options.onMissingTargets) {
              try {
                options.onMissingTargets(missingSteps.map((s) => s.element));
              } catch (e) {
                /* ignore */
              }
            }
          ...
          if (!allowPartial) {
            console.warn(
              'Tour start requires all steps but some are missing:',
              missingSteps.map((s) => s.element),
            );
            if (opts.onMissingTargets) {
              try {
          ...
          console.warn(
            'Some tour steps are missing; starting with available steps:',
            presentSteps.map((s) => s.element),
          );
          if (opts.onPartialStart) {
            try {
              opts.onPartialStart(missingSteps.map((s) => s.element));
            } catch (e) {
              /* ignore */
            }
          }
        ...
        try {
          ...
          if (driveError.message?.includes('No such element')) {
            errorMessage =
              'Some elements on this page are not ready yet. Please try again in a moment.';
            errorCode = 'ELEMENT_MISSING';
          } else if (driveError.message?.includes('driver')) {
            errorMessage =
              'Tour system is temporarily unavailable. Please refresh the page and try again.';
            errorCode = 'DRIVER_ERROR';
          } else if (
            driveError.message?.includes('permission') ||
            driveError.message?.includes('access')
          ) {
        ...
        return {
          ...
          message: 'A critical error occurred while starting the tour.',
