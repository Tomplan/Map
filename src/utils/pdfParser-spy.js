...
function parseSpatialInvoice(items, allowedItems) {
  ...
  lines.forEach((lineItems, index) => {
    ...
    if (!parsed.invoice_number) {
      // Look for explicit "Factuurnummer" and grab the exact line that follows it 1-4 items down
      const fNumIndex = lineItems.findIndex(
        (i) =>
          i.str.toLowerCase().includes('factuurnummer') ||
          i.str.toLowerCase().includes('factuur nr'),
      );
      if (fNumIndex >= 0) {
        for (let i = 1; i <= 4; i++) {
          if (lineItems[fNumIndex + i] && /^\d{5,}$/.test(lineItems[fNumIndex + i].str.trim())) {
            parsed.invoice_number = 'INDEX_0_' + lineItems[fNumIndex + i].str.trim();
            break;
          }
        }
      }

      // Inline regex fallback (if Factuurnummer and the number are on the same string chunk)
      if (!parsed.invoice_number) {
        const match = textChunk.match(/(?:Factuurnummer|Factuur nr)[:\.\s]+([A-Z0-9-]+)/i);
        if (match) {
          parsed.invoice_number = 'REGEX_1_' + match[1].trim();
        }
      }
      ...
      if (!parsed.invoice_number) {
        ...
        if (fItem) parsed.invoice_number = 'REGEX_2_' + fItem.str.trim();
      }
    }
