  ...
  lines.forEach((lineItems, index) => {
    ...
    if (!parsed.invoice_number) {
      if (lowerText.includes('factuurnummer') || lowerText.includes('factuur nr')) {
        ...
        if (lines[index + 1]) {
           // We look at individual chunks on the next line to avoid merging Date & Invoice No.
           const potentialNumbers = lines[index + 1].map(i => i.str.trim()).filter(str => str.length >= 4);
           
           // Find the chunk that is made purely of digits and/or typical invoice chars
           const validChunk = potentialNumbers.find(str => /^[\d-]+$/.test(str) || /\d{4,}/.test(str));
           
           if (validChunk) {
               const match = validChunk.match(/([A-Z0-9-]+)/i);
