  ...
  lines.forEach((lineItems, index) => {
    ...
    if (!parsed.invoice_number) {
      if (lowerText.includes('factuurnummer') || lowerText.includes('factuur nr')) {
        ...
        if (lines[index + 1]) {
           const nextLineText = lines[index + 1].map(i => i.str).join(' ').trim();
           // Remove spaces in case of weird kerning and grab the numbers
           const cleanNextLine = nextLineText.replace(/\s+/g, '');
           const match = cleanNextLine.match(/(\d{5,})/);
           if (match) {
             parsed.invoice_number = match[1];
           } else {
             // Hard fallback to just clean string on the very next line
             parsed.invoice_number = cleanNextLine.replace(/[^A-Z0-9-]/gi, '');
           }
        }
